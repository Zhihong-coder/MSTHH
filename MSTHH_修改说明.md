...
原始代码基于 **STAEformer**，本次改动将其完全替换为论文中的 **MSTHH** 模型。共新增/修改 4 个文件。
...
MSTHH/
├── model/
│   ├── STAEformer.py        ← 原始模型（保留，不动）
│   ├── STAEformer.yaml      ← 原始配置（保留，不动）
│   ├── MSTHH.py             ← ★ 新增：MSTHH 完整模型实现
│   ├── MSTHH.yaml           ← ★ 新增：MSTHH 配置文件
│   └── train_msthh.py       ← ★ 新增：MSTHH 专用训练脚本
└── lib/
    ├── data_prepare.py      ← 原始数据加载（保留，不动）
    └── data_prepare_multimodal.py  ← ★ 新增：多模态数据加载
```
